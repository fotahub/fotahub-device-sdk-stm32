...
void blink_init(uint16_t blinkPeriod)
{
   ...
   */
  hTimer.Init.Prescaler = ((uint32_t)((((uint32_t)(BlinkDrv_Hz_to_MHz_any(((int64_t)((SystemCoreClock / 2)))))) - 1)));
  hTimer.Init.CounterMode = TIM_COUNTERMODE_UP;
  hTimer.Init.Period = (((uint32_t)(BlinkDrv_ms_to_micros_any(((int64_t) blinkPeriod)))) >> 1) - 1;
